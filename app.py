...
if menu == ":green[Researcher Profile]":
    ...
    if data_option == "Radar Observations":
        ...
        filtered_weather['Date'] = filtered_weather['Day'].astype(str).str.cat(filtered_weather['Month'].astype(str), sep='-').str.cat(filtered_weather['Year'].astype(str), sep='-')
        fig = px.line(filtered_weather, x=filtered_weather.Date, y=filtered_weather.Streamflow, title="Streamflow on Vaal River")
        #fig.update_xaxes(tickvals=filtered_weather.Year) #title_text='Date', 
